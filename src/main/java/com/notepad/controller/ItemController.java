 ...
 * @since   2021-04-22
 */
@Api(value="Notes", description="Operations pertaining to notes", tags = { "Notes" })
@RestController
...
public class ItemController {
	...
	public ResponseEntity<List<ItemDTO>> saveItem(@RequestBody List<ItemDTO> itemDTO) {
		log.info("Rest request to save Item: {}", itemDTO);
		return ResponseEntity.ok().body(itemService.save(itemDTO, getPrincipal()));
	}

	/**
	 * {@code PUT  /api/menu/updateItem} : Updates existing item/items.
	 *
	 * @param itemDTOs the itemDTO to update.
	...
	public ResponseEntity<List<ItemDTO>> updateItem(@RequestBody List<ItemDTO> itemDTOs) {
		...
		return ResponseEntity.ok().body(itemService.save(itemDTOs, getPrincipal()));
	}
	...
	public ResponseEntity<List<ItemDTO>> getAllItemsByUserId() {
		log.info("Rest request to get all Items from menu with userId : {}");
		List<ItemDTO> itemDTOs = itemService.findAllByUserId(getPrincipal());
		return ResponseEntity.ok().body(itemDTOs);
	}
	/**
	...
	public ResponseEntity<Map<String, String>> deleteItem(@PathVariable Long itemId) {
		...
		itemService.delete(itemId,getPrincipal());
